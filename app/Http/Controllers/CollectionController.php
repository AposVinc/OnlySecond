...
class CollectionController extends Controller
{
    ...
    {
        if (Collection::withoutTrashed()->exists()){
            $selected_collection = Collection::where('name',$name)->first();

            $brands = Brand::all();
            return view('backend.collection.edit',['brands' => $brands, 'selected_collection' => $selected_collection]);
        } else {
            return redirect()->to('Admin/Collection/List')->with('error','Non ci sono Collezioni da Modificare!!');
        }
    }
    ...
    public function restore(Request $request)   //query senza nome
    {
        ...
        if (!$collection->brand->trashed()){
            ...
            return redirect()->to('Admin/Collection/List')->with('error', 'Errore!! Il Brand della collezione è disattivato!!');
        }
    }

    public function restoreButton($name)
    {
        ...
        if (!$collection->brand->trashed()){
            ...
            return redirect()->back()->with('error', 'Errore!! Il Brand della collezione è disattivato!!');
        }
    }
        ...
        if(Banner::withoutTrashed()->where('collection_id', $idcollection)->exists()){
            ...
            if(Banner::withoutTrashed()->where('collection_id', $idcollection)->where('visible', true)->count('visible')){
                ...
                foreach ($types as $type){    //analizzo solo le situazioni di rischio
                    ...
                    $countVisible = $countVisibleTot - $coutnVisibleCollection;
                    if($countVisible<=1) {
                        return redirect()->back()->with('error', 'Errore durante l\'Eliminazione. Almeno un Banner di Tipo '. $type. ' deve rimanere Visiile dopo l\'eliminazione!!');
                    }
                }
