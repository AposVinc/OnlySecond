...
class PageController extends Controller
{
    ...
    public function editAbout(Request $request){
        ...
        $this->saveImg($request,'main-photo');
        $this->saveImg($request,'first-member-photo');
        $this->saveImg($request,'second-member-photo');
        $this->saveImg($request,'third-member-photo');

        $updated = DB::table('pages')->where('about',1)->update([
    ...
    public function saveImg(Request $request, string $name){
        ...
        if ($request->hasFile($name)){
            if ($request->file($name)->isValid()){
                ...
                $filename = $name . '.jpg';
                $path = $request->file($name)->storeAs($path, $filename);
                if($path==""){
                    return redirect()->to('Admin/Page/About')->with('error', 'Errore durante il caricamento. Riprovare!!!!');
                }
            }else{
